...
$opt_label_2             = ( '0' === $option['label_2'] ) || ! empty( $option['label_2'] ) ? $option['label_2'] : '';
...
<div class="wc-pao-addon-option-row">
	...
	<div class="wrapper-labels">
		<div class="wc-pao-addon-content-label <?php echo esc_attr( $opt_label_column ); ?>">
			<p class="label-title">תיאור</p>
			<textarea rows="2" type="text" name="product_addon_option_label[<?php echo $loop; ?>][]" value="<?php echo wp_unslash(esc_attr( $opt_label )); ?>" placeholder="תיאור"><?php echo wp_unslash(esc_attr( $opt_label )); ?></textarea>
		</div>
		
		<div class="wc-pao-addon-content-label <?php echo esc_attr( $opt_label_column ); ?>">
			<p class="label-title">מידע נוסף</p>
			<textarea rows="2" type="text" name="product_addon_option_label_2[<?php echo $loop; ?>][]" value="<?php echo wp_unslash(esc_attr( $opt_label_2 )); ?>" placeholder="מידע נוסף"><?php echo wp_unslash(esc_attr( $opt_label_2 )); ?></textarea>
		</div>
	</div>
	...
	<div class="wc-pao-addon-content-price-type">
		<select name="product_addon_option_price_type[<?php echo $loop; ?>][]" class="wc-pao-addon-option-price-type">
			<option <?php selected( 'flat_fee', $opt_price_type ); ?> value="flat_fee"><?php esc_html_e( 'Flat Fee', 'woocommerce-product-addons-extra-digital' ); ?></option>
			<option <?php selected( 'quantity_based', $opt_price_type ); ?> value="quantity_based"><?php esc_html_e( 'Quantity Based', 'woocommerce-product-addons-extra-digital' ); ?></option>
			<option <?php selected( 'percentage_based', $opt_price_type ); ?> value="percentage_based"><?php esc_html_e( 'Percentage Based', 'woocommerce-product-addons-extra-digital' ); ?></option>
		</select>
	</div>
