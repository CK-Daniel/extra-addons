...
<?php foreach ( $addon['options'] as $i => $option ) {
	...
	?>
		<a href="#" title="<?php echo esc_attr( $option['label_2']); ?>" class="wc-pao-addon-image-swatch" data-value="<?php echo sanitize_title( $option['label'] ) . '-' . $loop; ?>" data-price="<?php echo esc_attr( '<span class="wc-pao-addon-image-swatch-price">' . wptexturize( wp_unslash($option['label']) ) . ' ' . $price_display . '</span>' ); ?>">
			<img src="<?php echo esc_url( $image_src[0] ? $image_src[0] : wc_placeholder_img_src() ); ?>" />
			
			<span class="option-info <?php if($option['label_2']) echo "has-info" ?>" style="white-space: pre-line"><?=wp_unslash($option['label'])?></span>
