...
<?php
	foreach ( $addon['options'] as $i => $option ) {
		...
		?>
		<p class="form-row form-row-wide wc-pao-addon-wrap wc-pao-addon-<?php echo sanitize_title( $field_name ); ?> addon-option" 
		   data-option-value="<?php echo esc_attr( sanitize_title( $label ) ); ?>"
		   ...
		   data-option-index="<?php echo esc_attr( $loop ); ?>">
			<label>
				<input type="radio" 
					...
					value="<?php echo sanitize_title( $label ); ?>" 
					...
					data-option-key="<?php echo esc_attr( sanitize_title( $label ) ); ?>"
					data-option-index="<?php echo esc_attr( $loop ); ?>"
					data-addon-field-name="<?php echo esc_attr( $field_name ); ?>" />&nbsp;&nbsp;<?php echo wptexturize( $label . ' ' . $price_for_display ); ?>
			</label>
		</p>
<?php } ?>
